...
const Candidatures = () => {
  ...
  const handleMatching = async () => {
    ...
    for (const candidature of candidatures) {
      ...
      try {
        const response = await axios.post('http://localhost:8081/api/match', {
          userId,
          jobId,
        });
        newPredictions.push({
          ...candidature,
          matchingData: response.data.matchingPercentage,
        });
      } catch (error) {
        console.error('Error matching candidate to job offer:', error);
      }
    }
    // Sort the predictions in descending order of the matching score
