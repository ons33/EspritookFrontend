...
class Pagetitle extends Component {
    render() {
        const {title,nomJour,typeJour,dateJour} = this.props;
        return (
            <div className="card shadow-xss w-100 d-block d-flex border-0 p-4 mb-3">
                <h2 className="fw-700 mb-0 mt-0 font-md text-grey-900 d-flex align-items-center">{title} - {nomJour}
                ...
                <p className="review-link font-xssss fw-500 text-grey-500 lh-3"> {dateJour } </p>
                <h4 className="text-danger font-xssss fw-700 ls-2">{typeJour}</h4>
            </div>
        );
    }
}
